...
srun \
  ...
  --pty \
  --container-image=$CONTAINER_IMAGES/claire+moalla+template-project-name+arm64-cuda-root-latest.sqsh \
...
srun \
  ...
  --nv \
  --writable-tmpfs \
  $CONTAINER_IMAGES/template-project-name_arm64-cuda-root-latest.sif \
  bash
